...
main(int argc, char **argv)
{
	...
	printf("number of bytes pushed to the stack between setcontext_called "
	       "and err = %ld\n", (long int) ((long long int) &setcontext_called - (long long int) &err));
	...
	printf("number of bytes between err and the saved stack in mycontext "
	       "= %ld\n", (long int) ((long long int) &err - (long long int) mycontext.uc_mcontext.gregs[REG_RSP]));
	...
	printf("value of uc_stack.ss_sp = 0x%lx\n",
	       (long unsigned int) mycontext.uc_stack.ss_sp);
...
show_interrupt(void)
{
	...
	printf("interrupt is disabled = %d\n",
	       (int) sigismember(&mycontext.uc_sigmask, SIGALRM));
